"""Definiciones exportadas del notebook S2. No editar ambas copias por separado."""

# %% Configuración
# CAMBIO S2: rutas del repositorio y configuración compartida, sin pedir claves al importar.
import ast
import hashlib
import json
import os
...
MAX_TOKENS = int(os.getenv("MAX_TOKENS_10K", "1024"))
K = 5
...
def modelo():
    ...
    return init_chat_model(
        MODELO, temperature=0, max_tokens=MAX_TOKENS, max_retries=0,
...
# CAMBIO S2: el filtro busca primero en todo el índice; no filtra sólo el top-k.
...
# %% Fusión BM25 y denso
# CAMBIO S2: RRF suma posiciones, nunca las puntuaciones de distinta escala.
...
# CAMBIO S2: reescritura real del LLM, sin traducciones sacadas de las respuestas conocidas.
def reescribir(consulta: str, config=None) -> str:
    ...
    if not r.text.strip():
        raise ValueError("El modelo devolvió una consulta vacía")
    return r.text.strip()
...
# CAMBIO S2: mismo contrato; XBRL conserva decimales y la búsqueda usa las mejoras.
@tool
...
# CAMBIO S2: números de la prosa y campo cifra; una corrección y después abstención.
...
# CAMBIO S2: cargar sólo definiciones del notebook propio, sin ejecutar sus demostraciones.
...
# CAMBIO S2: literalidad + procedencia + juicio de soporte; cifra contra el parquet.
...
def juzgar_cita(item, resultado):
    ...
    juicio = modelo().with_structured_output(JuicioCita).invoke([
        {"role": "system", "content": (
    ...
    return {**como_dict(juicio), **registro.resumen()}
...
# CAMBIO S2: ruta JSONL, trazas completas y resultados reproducibles por ejecución.
    ...
    for item in preguntas:
        ...
        if not parar:
            try:
                ...
                if citas_de(respuesta_de(r)) and cita_correcta(item, r) is not False and juzgar:
